...
describe('getIconByName', () => {
  const peekIcon = vi.fn();
  const plugin = {
    getIconPackManager: () => ({ peekIcon }),
  } as any;
  ...
  it('should return the correct icon for a given name', () => {
    peekIcon.mockReturnValueOnce('IbTest');
    expect(icon.getIconByName(plugin, 'IbTest')).toBe('IbTest');
    expect(peekIcon).toHaveBeenCalledWith('IbTest');
  });

  it('should return `null` when no icon was found', () => {
    peekIcon.mockReturnValueOnce(undefined);
    expect(icon.getIconByName(plugin, 'IbFoo')).toBe(null);
  });
});
