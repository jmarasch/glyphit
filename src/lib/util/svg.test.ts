...
describe('colorize with hard-coded shape colors', () => {
  it('should recolor a fill declared on a child path', () => {
    // The shape of game-icons.net icons: color sits on the path, not the root.
    const input =
      '<svg viewBox="0 0 512 512"><path fill="#fff" d="M1 1z"/></svg>';

    const output = svg.colorize(input, '#ff8800');

    expect(output).toContain('fill="#ff8800"');
    expect(output).not.toContain('#fff');
  });

  it('should recolor every shape of a multi-shape icon', () => {
    const input =
      '<svg><path fill="#fff" d="M1 1z"/><circle stroke="#000" cx="1"/></svg>';
  ...
  it('should preserve `none` so outline icons stay outlines', () => {
    const input =
      '<svg fill="none" stroke="#000"><path fill="none" stroke="#000"/></svg>';

    const output = svg.colorize(input, 'blue');

    expect(output).toContain('fill="none"');
    expect(output).toContain('stroke="blue"');
    expect(output).not.toContain('stroke="#000"');
  });

  it('should give an unpainted icon a color to inherit', () => {
    const input = '<svg viewBox="0 0 24 24"><path d="M1 1z"/></svg>';
