...
describe('getSuggestions', () => {
  const entryFor = (prefix: string, name: string) => ({
    pack: { getPrefix: () => prefix },
    entry: { id: `${prefix}${name}`, name },
  });

  beforeEach(() => {
    vi.restoreAllMocks();
    plugin.getIconPackManager = () => ({
      getAllEntries: () => [
        entryFor('Ib', 'WinkingFace'),
        entryFor('Ib', 'Heart'),
      ],
    });
  });

  it('should match icons from the pack index by name', () => {
    suggestionIcon.context = {
      ...suggestionIcon.context,
      query: ':winkingface',
    };

    expect(suggestionIcon.getSuggestions(suggestionIcon.context)).toContain(
      'IbWinkingFace',
    );
  });

  it('should include emoji shortcodes alongside icons', () => {
    suggestionIcon.context = {
      ...suggestionIcon.context,
      query: ':heart',
    };
    ...
    expect(suggestions).toContain('IbHeart');
    expect(suggestions.length).toBeGreaterThan(1);
  });

  it('should return no icons for a query that matches nothing', () => {
    suggestionIcon.context = {
      ...suggestionIcon.context,
      query: ':zzzznotanicon',
    };

    expect(suggestionIcon.getSuggestions(suggestionIcon.context)).toEqual([]);
  });
});
